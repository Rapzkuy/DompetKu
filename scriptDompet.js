...
let wallets = JSON.parse(localStorage.getItem("dompetku-wallets") || "null");
if (!wallets || !wallets.length) {
  wallets = [{ id: 1, name: "Dompet", emoji: "💵", color: "#c9a84c" }];
  localStorage.setItem("dompetku-wallets", JSON.stringify(wallets));
}
let transactions = JSON.parse(localStorage.getItem("dompetku") || "[]");
let _quickBtns = JSON.parse(localStorage.getItem("dompetku-qbtns") || "[]");
let _qbtnEditMode = false;
...
============================================================ */
let expRecurringList = JSON.parse(
  localStorage.getItem("dompetku-exp-recurring") || "[]",
);
let expRecFormOpen = false;
function simpanExpRutin() {
  localStorage.setItem(
    "dompetku-exp-recurring",
    JSON.stringify(expRecurringList),
  );
}
function toggleExpRecForm() {
  expRecFormOpen = !expRecFormOpen;
  document.getElementById("exp-rec-form").style.display = expRecFormOpen
    ? "block"
    : "none";
  document.getElementById("exp-rec-toggle-btn").textContent = expRecFormOpen
    ? "\u00d7 Tutup"
...
function addExpRecurring() {
  ...
  if (!name || !amount || amount <= 0 || !start) {
    showToast("\u26a0\ufe0f Lengkapi semua field!");
    return;
  }
  expRecurringList.push({
    id: Date.now(),
    name,
    amount,
    freq,
...
function cekExpRutin() {
  ...
  expRecurringList.forEach((rec) => {
    ...
    if (rec.freq === "weekly") jatuh = Math.floor((today - ref) / 864e5) >= 7;
      ...
      jatuh = today.getFullYear() !== ref.getFullYear();
  ...
  if (ada) {
    simpanData();
    simpanExpRutin();
    showToast("\ud83d\udd34 Pengeluaran rutin otomatis dicatat!");
  }
}
...
function exportCSV() {
  if (!transactions.length) {
    showToast("\u26a0\ufe0f Belum ada data untuk diexport!");
    return;
  }
  ...
  a.href = url;
  a.download = "dompetku-" + toDateStr(new Date()) + ".csv";
...
function simpanGoals() {
  localStorage.setItem("dompetku-goals", JSON.stringify(goals));
}
function toggleGoalForm() {
  goalFormOpen = !goalFormOpen;
  document.getElementById("goal-form-panel").style.display = goalFormOpen
    ? "block"
    : "none";
  document.getElementById("goal-toggle-btn").textContent = goalFormOpen
    ? "\u00d7 Tutup Form Target"
...
function addGoal() {
  ...
  if (!name || !target || target <= 0) {
    showToast("\u26a0\ufe0f Lengkapi nama dan nominal target!");
    return;
  }
...
let recurringList = JSON.parse(
  localStorage.getItem("dompetku-recurring") || "[]",
);
/*
...
function handleFoto(e) {
  if (e.target.files[0]) prosesGambar(e.target.files[0]);
}
...
function prosesGambar(file) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      const sb = Math.round(file.size / 1024),
        ss = Math.round((fotoBase64.length * 0.75) / 1024);
...
function simpanData() {
  localStorage.setItem("dompetku", JSON.stringify(transactions));
}
function simpanRutin() {
  localStorage.setItem("dompetku-recurring", JSON.stringify(recurringList));
}
...
function addTransaction() {
  ...
  const walletId =
    parseInt(document.getElementById("wallet-select")?.value) || wallets[0].id;
  transactions.unshift({
    id: Date.now(),
    type,
    desc,
...
function toggleRecForm() {
  ...
  document.getElementById("rec-form").style.display = recFormOpen
    ? "block"
    : "none";
  document.getElementById("rec-toggle-btn").textContent = recFormOpen
    ? "× Tutup"
    : "＋ Tambah Pemasukan Rutin";
}
...
function addRecurring() {
  ...
  if (!name || !amount || amount <= 0 || !start) {
    showToast("⚠️ Lengkapi semua field rutin!");
    return;
  }
...
function renderList() {
  ...
  if (searchVal)
    filtered = filtered.filter((t) => t.desc.toLowerCase().includes(searchVal));
  ...
  if (!filtered.length) {
    el.innerHTML =
      '<div class="empty-msg">Tidak ada transaksi ditemukan 🌱</div>';
    return;
  }
  el.innerHTML = filtered
    ...
    <div class="tx-item">
      ...
      <div class="tx-info">
        <div class="tx-desc">
  ${tx.desc}
  ${tx.isRecurring ? '<span class="badge-recurring">rutin</span>' : ""}
  
 
  ${(() => {
    const w = wallets.find((x) => x.id === tx.walletId);
    return w
      ? `<span class="badge-wallet" style="background:${w.color}22;color:${w.color}">${w.emoji}</span>`
      : "";
  })()}
</div>
...
function renderLineChart() {
  ...
  if (chartPeriod === "minggu") {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const lbl =
        BULAN[d.getMonth()] + " " + d.getFullYear().toString().slice(2);
      labels.push(lbl);
      byLabel[lbl] = {
        income: 0,
        expense: 0,
    ...
    transactions.forEach((tx) => {
      ...
      const lbl = labels.find(
        (l) => byLabel[l].month === m && byLabel[l].year === y,
      );
...
function render() {
  ...
  renderGoals();
  // TAMBAHKAN DUA INI:
  _renderWalletCards(); // Supaya kartu saldo di atas update
  _renderQuickBtns(); // Supaya tombol cepat muncul
}
...
function renderCalendar() {
  ...
  dayNames.innerHTML = NAMA_HARI.map(
    (h) => `<div class="cal-day-name">${h}</div>`,
  ).join("");
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const tooltip =
      exp > 0 ? `<span class="cal-tooltip">📉 ${formatRupiah(exp)}</span>` : "";
...
function simpanIou() {
  localStorage.setItem("dompetku-iou", JSON.stringify(iouList));
}

function setIouTab(tab) {
  iouTab = tab;
  // Update tampilan tab
  document.getElementById("iou-tab-hutang").className =
    "iou-tab" + (tab === "hutang" ? " active-hutang" : "");
  document.getElementById("iou-tab-piutang").className =
    "iou-tab" + (tab === "piutang" ? " active-piutang" : "");
...
function toggleIouForm() {
  ...
  document.getElementById("iou-form").style.display = iouFormOpen
    ? "block"
    : "none";
}
...
function addIou() {
  ...
  if (!nama || !amount || amount <= 0) {
    showToast("⚠️ Lengkapi nama dan nominal!");
    return;
  }
...
function renderIou() {
  ...
  const totalHutang = iouList
    .filter((i) => i.tipe === "hutang" && !i.lunas)
    .reduce((s, i) => s + i.amount, 0);
  const totalPiutang = iouList
    .filter((i) => i.tipe === "piutang" && !i.lunas)
    .reduce((s, i) => s + i.amount, 0);
  const net = totalPiutang - totalHutang;

  document.getElementById("iou-total-hutang").textContent =
    formatRupiah(totalHutang);
  document.getElementById("iou-total-piutang").textContent =
...
function toggleCalc() {
  ...
  document.getElementById("calc-widget").style.display = calcWidgetOpen
    ? "block"
    : "none";
}
...
function calcEqual() {
  try {
    ...
    document.getElementById("calc-result").textContent = new Intl.NumberFormat(
      "id-ID",
    ).format(parseFloat(result.toFixed(10)));
    document.getElementById("calc-expr").textContent = calcExpr + " =";
...
function simpanWish() {
  localStorage.setItem("dompetku-wishlist", JSON.stringify(wishList));
}

function toggleWishForm() {
  wishFormOpen = !wishFormOpen;
  document.getElementById("wish-form").style.display = wishFormOpen
    ? "block"
    : "none";
  document.getElementById("wish-toggle-btn").textContent = wishFormOpen
    ? "× Tutup"
    : "＋ Tambah Wishlist";
}
...
function addWish() {
  ...
  if (!name || !price || price <= 0) {
    showToast("⚠️ Lengkapi nama dan harga!");
    return;
  }
  ...
  el.innerHTML = wishList
    .map((w) => {
      ...
      if (done) {
        ...
        const tglTarget = target.toLocaleDateString("id-ID", {
          day: "numeric",
          month: "short",
          year: "numeric",
        });
...
/*
================================================================
FILE INI: TAMBAHAN_JS.js
Paste SEMUA isi file ini ke paling BAWAH scriptDompet.js,
tepat sebelum baris terakhir:
  document.addEventListener('keydown', ...)

Lalu lakukan 4 edit kecil di scriptDompet.js yang sudah ada
(instruksi ada di bagian bawah file ini).
================================================================
*/

/* ============================================================
   FITUR 1 — MULTI-WALLET (LOKASI UANG)
   ============================================================
   Data: localStorage key 'dompetku-wallets'
   Format tiap wallet: { id, name, emoji, color }
   Tiap transaksi ditambah field walletId.

   Saldo per wallet dihitung dengan memfilter transactions
   berdasarkan walletId.
============================================================ */

// State warna yang sedang dipilih di modal
let _wmColor = WALLET_COLORS[0];

// Load data wallet dari localStorage
// Jika kosong, buat satu default "Dompet"

function _simpanWallets() {
  localStorage.setItem("dompetku-wallets", JSON.stringify(wallets));
}
...
function openWalletModal() {
  ...
  document.getElementById("wm-emoji").value = "💵";
  _wmColor = WALLET_COLORS[0];

  // Render pilihan warna
  document.getElementById("wm-color-row").innerHTML = WALLET_COLORS.map(
    (c) =>
      `<div class="wcp-dot ${c === _wmColor ? "selected" : ""}"
          style="background:${c}"
          onclick="_selectWmColor('${c}',this)"></div>`,
  ).join("");
...
function _selectWmColor(color, el) {
  _wmColor = color;
  document
    .querySelectorAll("#wm-color-row .wcp-dot")
...
function saveWallet() {
  ...
  const emoji = document.getElementById("wm-emoji").value.trim() || "💵";
  if (!name) {
    showToast("⚠️ Isi nama lokasi dulu!");
    return;
  }

  wallets.push({ id: Date.now(), name, emoji, color: _wmColor });
  _simpanWallets();
  _renderWalletCards();
  _updateWalletSelect();
  closeWalletModal();
...
function deleteWallet(id) {
  if (wallets.length <= 1) {
    showToast("⚠️ Minimal harus ada 1 lokasi!");
    return;
  }
  if (!confirm("Hapus lokasi ini?\nTransaksi terkait tetap tersimpan.")) return;
  wallets = wallets.filter((w) => w.id !== id);
  _simpanWallets();
  _renderWalletCards();
  _updateWalletSelect();
  showToast("🗑️ Lokasi dihapus.");
}

/* Hitung saldo per wallet dari semua transaksi */
function _getSaldoPerWallet() {
  const map = {};
  wallets.forEach((w) => {
    map[w.id] = 0;
  });
  transactions.forEach((tx) => {
    // Transaksi lama tanpa walletId dianggap masuk ke wallet pertama
    const wid = tx.walletId || wallets[0].id;
    if (map[wid] === undefined) map[wid] = 0;
    map[wid] += tx.type === "income" ? tx.amount : -tx.amount;
  });
  return map;
}

/* Render baris kartu wallet */
function _renderWalletCards() {
  const container = document.getElementById("wallet-cards");
  if (!container) return;

  const saldos = _getSaldoPerWallet();

  container.innerHTML = wallets
    .map((w) => {
      const saldo = saldos[w.id] || 0;
      const color = w.color || "#c9a84c";
      const saldoColor = saldo >= 0 ? "var(--green)" : "var(--red)";
      return `
      <div class="wallet-card">
        <div class="wallet-card-bar" style="background:${color}"></div>
        <button class="wc-del" onclick="deleteWallet(${w.id})" title="Hapus">×</button>
        <div class="wc-label">Lokasi</div>
        <div class="wc-name">${w.emoji} ${w.name}</div>
        <div class="wc-amount" style="color:${saldoColor}">${formatRupiah(saldo)}</div>
      </div>`;
    })
    .join("");
}

/* Update dropdown pilih wallet di form */
function _updateWalletSelect() {
  ...
  if (wallets.find((w) => String(w.id) === String(prev))) sel.value = prev;
}

/* Tutup modal wallet jika klik backdrop */
// Ganti bagian yang ada addEventListener untuk modal
...
const qbtnModal = document.getElementById("qbtn-modal");
if (qbtnModal) {
  qbtnModal.addEventListener("click", function (e) {
    if (e.target === this) closeQbtnModal();
  });
}

/* ============================================================
   FITUR 2 — TOMBOL CEPAT (QUICK BUTTONS)
   ============================================================
   Data: localStorage key 'dompetku-qbtns'
   Format tiap tombol: { id, label, amount, type }

   Klik tombol → isi otomatis:
     - select#type  → tipe tombol
     - input#amount → nominal (sudah terformat)
     - input#desc   → label tombol (bisa diedit)
   Lalu fokus ke input desc.
============================================================ */

function _simpanQbtns() {
  localStorage.setItem("dompetku-qbtns", JSON.stringify(_quickBtns));
}

/* Toggle mode edit (tampilkan tombol × di tiap tombol cepat) */
function toggleQbtnEdit() {
  _qbtnEditMode = document.getElementById("qbtn-edit-chk").checked;
  _renderQuickBtns();
}

/* --- Modal tambah quick button --- */
function openQbtnModal() {
  document.getElementById("qm-label").value = "";
  document.getElementById("qm-amount").value = "";
  document.getElementById("qm-type").value = "expense";
  document.getElementById("qbtn-modal").classList.add("open");
  setTimeout(() => document.getElementById("qm-label").focus(), 50);
}

function closeQbtnModal() {
  document.getElementById("qbtn-modal").classList.remove("open");
}

function saveQbtn() {
  const label = document.getElementById("qm-label").value.trim();
  // Gunakan getRawAmount karena input pakai masking
  const amount = getRawAmount("qm-amount");
  const type = document.getElementById("qm-type").value;

  if (!label || !amount || amount <= 0) {
    showToast("⚠️ Isi label dan nominal!");
    return;
  }

  _quickBtns.push({ id: Date.now(), label, amount, type });
  _simpanQbtns();
  _renderQuickBtns();
  closeQbtnModal();
  showToast("⚡ Tombol cepat ditambahkan!");
}

function _deleteQbtn(id) {
  _quickBtns = _quickBtns.filter((b) => b.id !== id);
  _simpanQbtns();
  _renderQuickBtns();
}

/*
   applyQbtn(id) — isi form transaksi otomatis dari tombol cepat.
   Langkah:
   1. Set select#type ke tipe tombol
   2. Panggil updateCategories() agar kategori ikut berubah
   3. Set input#amount ke nominal (terformat dengan titik ribuan)
   4. Set input#desc ke label tombol
   5. Fokus ke desc agar user bisa langsung edit jika perlu
*/
function applyQbtn(id) {
  const btn = _quickBtns.find((b) => b.id === id);
  if (!btn) return;

  // 1. Set tipe
  document.getElementById("type").value = btn.type;
  // 2. Refresh kategori sesuai tipe baru
  updateCategories();
  // 3. Set nominal — format dengan titik ribuan agar masking konsisten
  document.getElementById("amount").value = new Intl.NumberFormat(
    "id-ID",
  ).format(btn.amount);
  // 4. Set keterangan
  document.getElementById("desc").value = btn.label;
  // 5. Fokus dan seleksi teks keterangan
  const descEl = document.getElementById("desc");
  descEl.focus();
  descEl.select();

  showToast(`⚡ ${btn.label} — ${formatRupiah(btn.amount)} terisi!`);
}

/* Render baris tombol-tombol cepat */
function _renderQuickBtns() {
  const row = document.getElementById("quick-btn-row");
  if (!row) return;

  const btnHtml = _quickBtns
    .map((b) => {
      const cls = b.type === "income" ? "q-income" : "q-expense";
      const sign = b.type === "income" ? "＋" : "－";
      // Tombol × hanya muncul saat mode edit aktif
      const delBtn = _qbtnEditMode
        ? `<button class="qbtn-x"
           onclick="event.stopPropagation();_deleteQbtn(${b.id})"
           title="Hapus tombol">×</button>`
        : "";
      return `<button class="qbtn ${cls}" onclick="applyQbtn(${b.id})">
      <span>${sign}</span>
      <span>${b.label}</span>
      <span style="opacity:.65;font-size:.7rem">${singkat(b.amount)}</span>
      ${delBtn}
    </button>`;
    })
    .join("");

  // Tombol "+ Buat Tombol" selalu ada di akhir
  row.innerHTML =
    btnHtml +
    `<button class="qbtn-add" onclick="openQbtnModal()">＋ Buat Tombol</button>`;
}

/* Tutup modal qbtn jika klik backdrop */
document.getElementById("qbtn-modal").addEventListener("click", function (e) {
  if (e.target === this) closeQbtnModal();
});
...
(function initNewFeatures() {
  // Setup input masking untuk field modal quick button
  setupInputMask("qm-amount");

  // Render wallet cards & update dropdown
  _renderWalletCards();
  _updateWalletSelect();
  ...
  // Pastikan modal tertutup saat Escape ditekan
  // (tambahan ke listener keydown yang sudah ada)
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      closeWalletModal();
      closeQbtnModal();
...
/*
================================================================
EDIT KECIL DI scriptDompet.js — HANYA 4 PERUBAHAN
================================================================

EDIT 1 — Tambah walletId saat simpan transaksi
--------------------------------------------------
Di fungsi addTransaction(), cari baris ini:
  transactions.unshift({ id:Date.now(), type, desc, amount, cat, date, foto:fotoBase64 });

Ganti menjadi:
  const walletId = parseInt(document.getElementById('wallet-select')?.value) || wallets[0].id;
  transactions.unshift({ id:Date.now(), type, desc, amount, cat, date, foto:fotoBase64, walletId });


EDIT 2 — Update wallet cards setelah tambah transaksi
--------------------------------------------------
Di fungsi addTransaction(), cari baris:
  renderGoals();
...
EDIT 3 — Update wallet cards setelah hapus transaksi
--------------------------------------------------
Di fungsi deleteTransaction(), cari baris:
  simpanData(); render();

Tambahkan SETELAH baris itu (baris baru):
  _renderWalletCards();


EDIT 4 — Tampilkan badge wallet di riwayat transaksi (opsional tapi disarankan)
--------------------------------------------------
Di fungsi renderList(), cari template literal transaksi.
Cari baris ini:
  <div class="tx-desc">${tx.desc}${tx.isRecurring?'<span class="badge-recurring">rutin</span>':''}

Ganti menjadi:
  <div class="tx-desc">${tx.desc}${tx.isRecurring?'<span class="badge-recurring">rutin</span>':''}${(()=>{const w=wallets.find(x=>x.id===tx.walletId);return w?`<span class="badge-recurring" style="background:${w.color}22;color:${w.color}">${w.emoji}</span>`:''})()}
